...
realisations = np.repeat(realisation_number, 6)

# 0 - 25 prerecon spline 
# 26 - 51 prerecon poly 
# 52 - 77 postrecon spline 
# 78 - 103 postrecon poly 

if realisation_number not in [0, 26, 52, 78]: # shuffling the realisations (don't shuffle for mean of mocks) 
    
    if recon == 'pre' and broadband_method == 'spline':
        realisations[1] = realisation_number + 1 
        realisations[2] = realisation_number + 2
        realisations[3] = realisation_number + 3 
        realisations[4] = realisation_number + 4 
        realisations[5] = realisation_number + 5 
        realisations = np.where(realisations > 25, realisations-24, realisations)
        
    elif recon == 'pre' and broadband_method == 'poly': 
        realisations[1] = realisation_number + 1 
        realisations[2] = realisation_number + 2
        realisations[3] = realisation_number + 3 
        realisations[4] = realisation_number + 4 
        realisations[5] = realisation_number + 5 
        realisations = np.where(realisations > 51, realisations-24, realisations)
        
    elif recon == 'post' and broadband_method == 'spline': 
        realisations[1] = realisation_number + 1 
        realisations[2] = realisation_number + 2
        realisations[3] = realisation_number + 3 
        realisations[4] = realisation_number + 4 
        realisations[5] = realisation_number + 5 
        realisations = np.where(realisations > 77, realisations-24, realisations)
        ...
        realisations[4] = realisation_number + 4 
        realisations[5] = realisation_number + 5 
        realisations = np.where(realisations > 103, realisations-24, realisations)
...
for item in fitters:
    ...
    weight, chain, model = fitters[item].load()[realisations[count]][1], fitters[item].load()[realisations[count]][2], fitters[item].load()[realisations[count]][4]
...
# save to a file 
df_fit.to_csv("/pscratch/sd/a/abbew25/combinedfits_secondgen_mocks_v1_2_shuffled/"+data+"_"+recon+"_"+broadband_method +"_" + str(realisation_number) + ".csv")